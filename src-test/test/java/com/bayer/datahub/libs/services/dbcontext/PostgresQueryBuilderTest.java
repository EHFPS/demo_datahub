...
class PostgresQueryBuilderTest {
    private final PostgresQueryBuilder builder = FactoryBuilder.newBuilder(Map.of(
    ...
    void buildUpsertQuery() {
        ...
        var expQuery = "insert into the_table(fieldA, fieldB) values (?, ?) " +
                "on conflict (the_key) do update set fieldA = ?, fieldB = ?";
